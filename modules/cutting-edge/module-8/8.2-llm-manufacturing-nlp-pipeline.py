...
def generate_maintenance_logs(n: int = 500, seed: int = RANDOM_SEED) -> pd.DataFrame:
    ...
    return pd.DataFrame({"text": logs, "severity": severities, "tool_area": tool_areas})
...
def generate_shift_reports(n: int = 200, seed: int = RANDOM_SEED) -> pd.DataFrame:
    ...
    return pd.DataFrame({"text": reports, "summary": summaries})
...
class ManufacturingNLPPipeline:
    ...
    def __init__(self, task: str = "classification", backend: str = "classical", target_type: str = "severity"):
        """
        Initialize pipeline.
    ...
    def _fit_classical_classification(self, texts: np.ndarray, targets: np.ndarray):
        ...
        self.model = LogisticRegression(random_state=RANDOM_SEED, max_iter=1000, class_weight="balanced")
    ...
    def _fit_transformers_classification(self, texts: np.ndarray, targets: np.ndarray):
        ...
        self.model = pipeline("text-classification", model=model_name, device=0 if torch.cuda.is_available() else -1)
        ...
        self.metadata = ModelMetadata(
            task=self.task, backend=self.backend, model_type=f"Transformers: {model_name}", target_names=target_names
        )
    ...
    def _fit_classical_summarization(self, texts: np.ndarray, summaries: np.ndarray):
        ...
        self.metadata = ModelMetadata(
            task=self.task, backend=self.backend, model_type="TF-IDF Extractive Summarization"
        )
    ...
    def _fit_transformers_summarization(self, texts: np.ndarray, summaries: np.ndarray):
        ...
        self.model = pipeline("summarization", model=model_name, device=0 if torch.cuda.is_available() else -1)

        self.metadata = ModelMetadata(task=self.task, backend=self.backend, model_type=f"Transformers: {model_name}")
    ...
    def _evaluate_classification(self, y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
        ...
        if self.target_type == "severity":
            ...
            severity_penalty = np.where(
                ...
                1000,  # High missed
                np.where(
                    (y_true == 1) & (y_pred == 0), 100, misclassification_costs * 10  # Medium missed  # Other errors
                ),
            )
    ...
    def _evaluate_summarization(self, y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
        ...
        length_similarity = 1.0 - np.mean(np.abs(np.array(true_lengths) - np.array(pred_lengths))) / np.mean(
            true_lengths
        )
...
def action_train(args) -> None:
    ...
    try:
        ...
        # Train model
        pipeline = ManufacturingNLPPipeline(task=args.task, backend=args.backend, target_type=args.target_type)
        ...
        error_result = {"status": "error", "error": str(e), "task": args.task, "backend": args.backend}
...
def action_evaluate(args) -> None:
    ...
    try:
        ...
        error_result = {"status": "error", "error": str(e), "model_path": args.model_path}
        print(json.dumps(error_result, indent=2))
        sys.exit(1)
    ...
    try:
        ...
        if pipeline.task == "classification":
            ...
            result = {
                ...
                "task": pipeline.task,
                "backend": pipeline.backend,
                "prediction": {"value": pred_value, "label": pred_label},
            }
        else:  # summarization
            result = {
                "status": "predicted",
...
Examples:
  ...
  python 8.2-llm-manufacturing-nlp-pipeline.py predict --model-path model.joblib --input-json '{"text":"Pump P-101 emergency shutdown due to overheating"}'
    ...
    train_parser.add_argument(
        "--task", choices=["classification", "summarization"], default="classification", help="Task type"
    )
    train_parser.add_argument(
        "--backend", choices=["classical", "transformers"], default="classical", help="Model backend"
    )
    train_parser.add_argument(
        "--target-type", choices=["severity", "tool_area"], default="severity", help="Target for classification"
    )
    train_parser.add_argument("--n-samples", type=int, default=500, help="Number of samples to generate")
    ...
    eval_parser.add_argument("--n-samples", type=int, default=200, help="Number of samples for evaluation")
    ...
    pred_parser.add_argument(
        "--input-json", required=True, help='JSON string with input data (must contain "text" field)'
    )
