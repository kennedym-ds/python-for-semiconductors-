...
class TestGANsPipeline(unittest.TestCase):
    ...
    def test_generate_samples(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            run_cmd(["train", "--epochs", "1", "--batch-size", "4", "--image-size", "32", "--save", str(model_path)])
    ...
    def test_evaluate_model(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Evaluate model
            result = run_cmd(["evaluate", "--model-path", str(model_path)])
    ...
    def test_save_load_roundtrip(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            generate_result = run_cmd(["generate", "--model-path", str(model_path), "--num-samples", "4"])

            generate_output = json.loads(generate_result.stdout)
            self.assertEqual(generate_output["status"], "generated")

            # Evaluate loaded model
            eval_result = run_cmd(["evaluate", "--model-path", str(model_path)])
    ...
    def test_cli_error_handling(self):
        ...
        result = run_cmd(["generate", "--model-path", "nonexistent.joblib"], check=False)
        ...
        result = run_cmd(["evaluate", "--model-path", "nonexistent.joblib"], check=False)
