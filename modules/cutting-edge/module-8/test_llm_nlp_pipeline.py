...
def test_train_and_evaluate_roundtrip(tmp_path):
    ...
    # Train and save
    train_out = run_cmd(['train', '--task', 'classification', '--backend', 'classical', 
                        '--n-samples', '30', '--save', str(model_path)])
...
def test_predict_classification(tmp_path):
    ...
    run_cmd(['train', '--task', 'classification', '--backend', 'classical',
            '--n-samples', '30', '--save', str(model_path)])
...
def test_predict_summarization(tmp_path):
    ...
    # Train and save model
    run_cmd(['train', '--task', 'summarization', '--backend', 'classical',
            '--n-samples', '20', '--save', str(model_path)])
...
def test_different_target_types(tmp_path):
    ...
    train_out = run_cmd(['train', '--task', 'classification', '--backend', 'classical',
                        '--target-type', 'tool_area', '--n-samples', '30', '--save', str(model_path)])
    assert train_out['status'] == 'trained'
...
def test_error_handling():
    ...
    result = subprocess.run([sys.executable, str(SCRIPT), 'predict',
                           '--model-path', 'nonexistent.joblib',
