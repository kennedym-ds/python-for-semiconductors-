...
class DocumentationReproducibilityPipeline:
    ...
    def generate_documentation(
        self, input_dir: Union[str, Path], output_dir: Union[str, Path], format: str = "markdown"
    ) -> Dict[str, Any]:
        ...
        if not input_path.exists():
            return {"success": False, "error": f"Input directory does not exist: {input_path}", "converted_files": []}
        ...
        for nb_file in notebook_files:
            try:
                ...
                if format in ["markdown", "both"]:
                    ...
                    converted_files.append({"source": str(nb_file), "output": str(md_output), "format": "markdown"})
                ...
                if format in ["html", "both"]:
                    ...
                    converted_files.append({"source": str(nb_file), "output": str(html_output), "format": "html"})

            except Exception as e:
                errors.append({"file": str(nb_file), "error": str(e)})
    ...
    def export_environment(self, output_path: Union[str, Path], format: str = "conda") -> Dict[str, Any]:
        ...
        try:
            if format == "conda":
                ...
                result = subprocess.run(
                    ["conda", "env", "export", "--no-builds"], capture_output=True, text=True, check=True
                )
                ...
                return {"success": False, "error": f"Unsupported format: {format}. Use conda, pip, or requirements"}
    ...
    def setup_mkdocs(self, project_dir: Union[str, Path]) -> Dict[str, Any]:
        ...
        mkdocs_config = {
            ...
            "nav": [
                ...
                {"Intermediate": [{"Module 4": "intermediate/module-4.md"}]},
                {"Project Development": [{"Module 10": "project-dev/module-10.md"}]},
            ],
            "markdown_extensions": [
                "codehilite",
                "admonition",
        ...
        try:
            ...
            return {"success": False, "error": f"mkdocs setup failed: {e}"}
    ...
    def build_docs(self, project_dir: Union[str, Path]) -> Dict[str, Any]:
        ...
        if not mkdocs_yml.exists():
            return {"success": False, "error": f"mkdocs.yml not found in {project_path}"}

        try:
            result = subprocess.run(["mkdocs", "build"], cwd=project_path, capture_output=True, text=True, check=True)

            site_dir = project_path / "site"
            return {"success": True, "site_dir": str(site_dir), "stdout": result.stdout, "build_time": "completed"}
    ...
    def save(self, path: Union[str, Path]) -> None:
        ...
        with open(save_path, "w") as f:
            json.dump(
                {"config": asdict(self.config) if self.config else None, "results": self.results},
                f,
                indent=2,
                default=str,
            )
...
def action_generate_docs(args) -> None:
    ...
    result = pipeline.generate_documentation(input_dir=args.input, output_dir=args.output, format=args.format)
...
def action_export_env(args) -> None:
    ...
    result = pipeline.export_environment(output_path=args.output, format=args.format)
...
Examples:
  ...
  python 10.3-documentation-reproducibility-pipeline.py build-docs \\
    ...
    p_generate = subparsers.add_parser("generate-docs", help="Convert notebooks to documentation")
    p_generate.add_argument("--input", required=True, help="Input directory containing notebooks")
    p_generate.add_argument("--output", required=True, help="Output directory for documentation")
    p_generate.add_argument(
        "--format", choices=["markdown", "html", "both"], default="markdown", help="Output format (default: markdown)"
    )
    p_generate.set_defaults(func=action_generate_docs)

    # validate-paths subcommand
    p_validate = subparsers.add_parser("validate-paths", help="Validate dataset path resolution")
    p_validate.add_argument("--modules-dir", required=True, help="Root directory containing modules")
    p_validate.set_defaults(func=action_validate_paths)

    # export-env subcommand
    p_export = subparsers.add_parser("export-env", help="Export environment for reproducibility")
    p_export.add_argument("--output", required=True, help="Output file path")
    p_export.add_argument(
        "--format", choices=["conda", "pip", "requirements"], default="conda", help="Export format (default: conda)"
    )
    p_export.set_defaults(func=action_export_env)

    # setup-mkdocs subcommand
    p_setup = subparsers.add_parser("setup-mkdocs", help="Set up mkdocs documentation site")
    p_setup.add_argument("--project-dir", required=True, help="Directory for mkdocs project")
    p_setup.set_defaults(func=action_setup_mkdocs)

    # build-docs subcommand
    p_build = subparsers.add_parser("build-docs", help="Build mkdocs documentation site")
    p_build.add_argument("--project-dir", required=True, help="Directory containing mkdocs project")
