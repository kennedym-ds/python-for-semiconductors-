...
try:
    ...
    spec = importlib.util.spec_from_file_location("doc_pipeline", "10.3-documentation-reproducibility-pipeline.py")
...
class TestDocumentationReproducibilityPipeline(unittest.TestCase):
    ...
    def test_generate_documentation_success(self):
        ...
        result = self.pipeline.generate_documentation(input_dir=nb_dir, output_dir=output_dir, format="markdown")
    ...
    def test_generate_documentation_missing_input(self):
        ...
        result = self.pipeline.generate_documentation(
            input_dir=self.temp_dir / "nonexistent", output_dir=self.temp_dir / "docs", format="markdown"
        )

        self.assertFalse(result["success"])
        # Check for either error type - depends on nbconvert availability
        self.assertTrue("does not exist" in result["error"] or "nbconvert not available" in result["error"])
            ...
            "cells": [
                {
                    ...
                    "metadata": {},
                    "outputs": [],
                    "source": ["# Bad pattern - flat dataset path\n", "data = pd.read_csv('datasets/secom.FLAT')"],
                }
            ],
            "metadata": {},
            "nbformat": 4,
    ...
    def test_validate_dataset_paths_missing_directory(self):
        ...
        result = self.pipeline.validate_dataset_paths(self.temp_dir / "nonexistent")
    ...
    def test_export_environment_conda_success(self, mock_run):
        ...
        mock_run.return_value = MagicMock(stdout="name: test\ndependencies:\n- python=3.9", returncode=0)

        output_path = self.temp_dir / "environment.yml"
        result = self.pipeline.export_environment(output_path=output_path, format="conda")
    ...
    def test_export_environment_pip_success(self, mock_run):
        ...
        mock_run.return_value = MagicMock(stdout="numpy==1.24.3\npandas==2.0.1", returncode=0)

        output_path = self.temp_dir / "requirements.txt"
        result = self.pipeline.export_environment(output_path=output_path, format="pip")
    ...
    def test_export_environment_invalid_format(self):
        ...
        result = self.pipeline.export_environment(output_path=output_path, format="invalid")
    ...
    def test_build_docs_success(self, mock_run):
        """Test mkdocs build."""
        mock_run.return_value = MagicMock(stdout="INFO - Documentation built", returncode=0)
    ...
    def test_save_and_load_pipeline(self):
        ...
        # Configure pipeline
        config = doc_pipeline.DocumentationConfig(input_dir=Path("test/input"), output_dir=Path("test/output"))
    ...
    def test_cli_help(self):
        ...
        try:
            result = subprocess.run(
                [sys.executable, str(self.script_path), "--help"], capture_output=True, text=True, timeout=30
            )
...
class TestDataClasses(unittest.TestCase):
    ...
    def test_documentation_config(self):
        ...
        config = doc_pipeline.DocumentationConfig(input_dir=Path("input"), output_dir=Path("output"), format="markdown")
    ...
    def test_environment_export(self):
        ...
        export = doc_pipeline.EnvironmentExport(
            format="conda", include_versions=True, output_path=Path("env.yml"), exclude_dev=False
        )
