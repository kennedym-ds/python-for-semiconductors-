...
class ScalingOptimizationPipeline:
    ...
    def fit(self, X: pd.DataFrame, y: Any):
        ...
        X_processed = self.timing_profiler.time_function(
            'preprocessing', self._preprocess_data, X
        )
        X_processed = self.memory_profiler.profile_function(
            'preprocessing', lambda x: x, X_processed
        )
        ...
        X_scaled = self.timing_profiler.time_function(
            'scaling', self.scaler.fit_transform, X_features
        )
        ...
        if self.use_incremental and hasattr(self.model, 'partial_fit'):
            ...
            self.timing_profiler.time_function(
                'training', self.model.fit, X_scaled, y_array
            )
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        if len(X_scaled) > self.batch_size and self.use_parallel:
            ...
            predictions = Parallel(n_jobs=self.n_jobs)(
                delayed(self.model.predict)(batch) for batch in batches
            )
...
def action_benchmark(args):
    ...
    results = {
        ...
        'speedup': profiler.get_times().get('loops', 0) / profiler.get_times().get('vectorized', 1) if 'loops' in profiler.get_times() else None
    }
...
def build_parser():
    ...
    p_train.add_argument('--strategy', default='vectorized', choices=['vectorized', 'loops'], help='Optimization strategy')
