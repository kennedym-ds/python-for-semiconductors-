...
def test_train_classical_svm():
    ...
    out = run_cmd(
        ["train", "--approach", "classical", "--model", "svm", "--epochs", "2", "--dataset", "synthetic_wafer_small"]
    )
...
def test_predict_single_wafer_classical():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        pred_out = run_cmd(["predict", "--model-path", str(model_path), "--input-json", input_json])
...
def test_class_imbalance_handling():
    ...
    classical_out = run_cmd(
        ["train", "--approach", "classical", "--model", "svm", "--dataset", "synthetic_wafer_small"]
    )
    dl_out = run_cmd(
        [
            "train",
...
def test_error_handling():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        try:
            run_cmd(["predict", "--model-path", str(model_path), "--input-json", '{"invalid": "input"}'])
        ...
        try:
            run_cmd(["predict", "--model-path", str(model_path), "--input-json", '{"wafer_map": "not_an_array"}'])
