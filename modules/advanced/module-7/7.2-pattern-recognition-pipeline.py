...
def generate_synthetic_wafer_maps(n_samples: int = 600, map_size: int = 64, 
                                  n_classes: int = 5, seed: int = RANDOM_SEED) -> Dict[str, Any]:
    ...
    for i in range(n_samples):
        ...
        if pattern_type == 0:  # Normal - random sparse defects
            ...
            wafer_map[center_mask & wafer_mask] = rng.binomial(1, defect_density, 
                                                              size=np.sum(center_mask & wafer_mask))
            ...
            ring_width = 4
            ring_mask = (np.abs(((x - center)**2 + (y - center)**2)**0.5 - ring_radius) <= ring_width) & wafer_mask
...
class ClassicalFeatureExtractor:
    ...
    def extract_texture_features(self, wafer_map: np.ndarray) -> np.ndarray:
        ...
        # Compute GLCM
        glcm = graycomatrix(wafer_uint8, distances=[1], angles=[0, np.pi/4, np.pi/2, 3*np.pi/4], 
    ...
    def extract_hog_features(self, wafer_map: np.ndarray) -> np.ndarray:
        ...
        hog_features = hog(wafer_map, orientations=4, pixels_per_cell=(16, 16),
    ...
    def extract_region_properties(self, wafer_map: np.ndarray) -> np.ndarray:
        ...
        return np.array([total_area, total_perimeter, n_components, 
                        largest_area, largest_perimeter, compactness])
...
if HAS_TORCH:
    class WaferMapDataset(Dataset):
        ...
        def __init__(self, wafer_maps: np.ndarray, labels: np.ndarray, 
...
class PatternRecognitionPipeline:
    ...
    def fit(self, wafer_maps: np.ndarray, labels: np.ndarray, wafer_ids: np.ndarray, 
...
def action_predict(args):
    ...
    proba_dict = {
        ...
        zip(pipeline.metadata.class_names, probabilities[0])
    }
...
def build_parser():
    ...
    p_train.add_argument('--dataset', default='synthetic_wafer', 
                        help='Dataset name (synthetic_wafer, wm811k)')
    p_train.add_argument('--approach', default='classical', choices=['classical', 'deep_learning'],
                        help='Modeling approach')
    p_train.add_argument('--model', default='svm',
                        help='Model type (svm, rf for classical; cnn for deep_learning)')
    ...
    p_train.add_argument('--no-focal-loss', action='store_true',
