...
def action_evaluate(args):
    ...
    try:
        ...
        if args.dataset == 'synthetic':
            images, annotations = generate_synthetic_wafer_defects(
                n_images=args.n_images,
                seed=RANDOM_SEED + 1000  # Different seed for test data
            )
        else:
            return {'error': f'Unknown dataset: {args.dataset}'}
...
Examples:
  ...
  Predict on image:
    ...
    p_train.add_argument('--backend', choices=['classical', 'yolo', 'fasterrcnn'], 
                        default='classical', help='Detection backend to use')
