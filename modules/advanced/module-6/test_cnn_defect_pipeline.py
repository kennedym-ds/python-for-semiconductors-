...
def test_train_simple_cnn():
    ...
    out = run_cmd(
        ["train", "--dataset", "synthetic_small", "--model", "simple_cnn", "--epochs", "2", "--batch-size", "16"]
    )
...
def test_save_load_roundtrip():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Load and evaluate
        eval_out = run_cmd(["evaluate", "--model-path", str(model_path), "--dataset", "synthetic_small"])
...
def test_predict_dataset():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Train model
        run_cmd(["train", "--dataset", "synthetic_small", "--epochs", "2", "--save", str(model_path)])

        # Predict on dataset
        pred_out = run_cmd(["predict", "--model-path", str(model_path), "--dataset", "synthetic_small"])
...
def test_predict_single_image():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        test_wafer = pipeline_module.generate_synthetic_wafer_map(pattern="center", size=64)
        ...
        pred_out = run_cmd(["predict", "--model-path", str(model_path), "--input-image", str(image_path)])
        assert pred_out["status"] == "predicted"
        assert "predictions" in pred_out
...
def test_synthetic_data_generator():
    ...
    for pattern in patterns:
        wafer = pipeline_module.generate_synthetic_wafer_map(pattern=pattern, size=32)
        assert wafer.shape == (32, 32)
        assert wafer.dtype == np.float32
        assert 0 <= wafer.min() <= wafer.max() <= 1

    # Test dataset generation
    images, labels, class_names = pipeline_module.generate_synthetic_dataset(n_samples=50, image_size=32)
...
def test_metrics_computation():
    ...
    out = run_cmd(["train", "--dataset", "synthetic_small", "--epochs", "2"])

    metrics = out["metrics"]
    required_metrics = ["accuracy", "f1_macro", "f1_weighted", "roc_auc_ovr", "pr_auc_macro", "pws", "estimated_loss"]
...
def test_deterministic_results():
    ...
    out1 = run_cmd(["train", "--dataset", "synthetic_small", "--epochs", "2"])

    out2 = run_cmd(["train", "--dataset", "synthetic_small", "--epochs", "2"])
...
def test_json_output_format():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Train first
        run_cmd(["train", "--dataset", "synthetic_small", "--epochs", "1", "--save", str(model_path)])
