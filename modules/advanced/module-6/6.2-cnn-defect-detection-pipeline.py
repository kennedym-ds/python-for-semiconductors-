...
def generate_synthetic_dataset(
    ...
    for i, pattern in enumerate(class_names):
        ...
        for j in range(n_pattern):
            ...
            wafer_map = generate_synthetic_wafer_map(
                pattern=pattern, 
                size=image_size, 
                seed=sample_seed
            )
    ...
    def evaluate(self, images: np.ndarray, labels: np.ndarray) -> Dict[str, float]:
        ...
        try:
            pr_auc = average_precision_score(
                y_encoded, probabilities, average='macro'
            )
        except ValueError:
            pr_auc = 0.0
...
def build_parser():
    parser = argparse.ArgumentParser(
        description='Module 6.2 - CNN Pipeline for Wafer Map Defect Detection'
    )
    subparsers = parser.add_subparsers(dest='command', required=True)
    
    # Train subcommand
    train_parser = subparsers.add_parser('train', help='Train a CNN model')
    train_parser.add_argument('--dataset', default='synthetic_wafer',
                            help='Dataset to use (synthetic_wafer, synthetic_small, wm811k)')
    train_parser.add_argument('--model', default='simple_cnn',
                            help='Model type (simple_cnn, cnn)')
    ...
    train_parser.add_argument('--learning-rate', type=float, default=0.001,
                            help='Learning rate')
    train_parser.add_argument('--fallback-model', default='random_forest',
                            help='Sklearn fallback model (random_forest, svm, logistic)')
    ...
    eval_parser.add_argument('--dataset', default='synthetic_wafer',
