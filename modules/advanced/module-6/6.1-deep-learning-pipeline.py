...
def generate_synthetic_tabular(
    n: int = 1000, n_features: int = 20, task: str = "regression", noise_level: float = 0.1, seed: int = RANDOM_SEED
) -> pd.DataFrame:
...
class DeepLearningPipeline:
    ...
    def fit(self, X: pd.DataFrame, y: np.ndarray) -> "DeepLearningPipeline":
        ...
        if self.backend == "pytorch":
            X_train, X_val, y_train, y_val = train_test_split(X_prep, y_prep, test_size=0.2, random_state=RANDOM_SEED)
        else:
            X_train, X_val, y_train, y_val = train_test_split(X_prep, y_prep, test_size=0.2, random_state=RANDOM_SEED)
        ...
        self.metadata = DeepLearningMetadata(
            ...
            training_params={"n_samples": len(X), "n_features": input_dim},
    ...
    def _fit_tensorflow(self, X_train, y_train, X_val, y_val, input_dim, output_dim):
        ...
        if self.task == "regression":
            self.model.compile(
                optimizer=tf.keras.optimizers.Adam(learning_rate=self.learning_rate), loss="mse", metrics=["mae"]
            )
        else:
            self.model.compile(
...
Examples:
  ...
  python 6.1-deep-learning-pipeline.py predict --model-path model.pth --input-json '{"temperature":455, "pressure":2.6}'
    ...
    train_parser.add_argument(
        "--dataset", required=True, choices=["synthetic_yield", "synthetic_defects"], help="Dataset to use for training"
    )
    train_parser.add_argument(
        "--backend", default="pytorch", choices=["pytorch", "tensorflow"], help="Deep learning backend"
    )
    train_parser.add_argument(
        "--task", default="regression", choices=["regression", "classification"], help="Learning task type"
    )
    train_parser.add_argument(
        "--hidden-dims", nargs="+", type=int, default=[64, 32, 16], help="Hidden layer dimensions"
    )
    train_parser.add_argument("--dropout-rate", type=float, default=0.2, help="Dropout rate")
    ...
    train_parser.add_argument("--n-samples", type=int, default=1000, help="Number of samples for synthetic data")
    ...
    eval_parser.add_argument(
        "--dataset", required=True, choices=["synthetic_yield", "synthetic_defects"], help="Dataset to evaluate on"
    )
    eval_parser.add_argument("--n-samples", type=int, default=500, help="Number of samples for synthetic data")
    ...
    predict_parser.add_argument("--input-json", required=True, help="JSON string with input features")
