...
def generate_synthetic_maintenance_data(
    ...
    for tool_id in range(n_tools):
        ...
        for hour_idx, ts in enumerate(timestamps):
            ...
            event_in_24h = any(maint_hour - hour_idx <= 24 and maint_hour > hour_idx for maint_hour in maint_times)
            event_in_72h = any(maint_hour - hour_idx <= 72 and maint_hour > hour_idx for maint_hour in maint_times)

            # Time to next event (for regression)
            times_to_events = [maint_hour - hour_idx for maint_hour in maint_times if maint_hour > hour_idx]
...
def create_time_window_features(
    df: pd.DataFrame, sensor_cols: List[str], window_sizes: List[int] = [6, 12, 24], lag_hours: List[int] = [1, 6, 12]
) -> pd.DataFrame:
    ...
    feature_cols = [
        ...
        if any(
            suffix in col for suffix in ["_mean_", "_std_", "_min_", "_max_", "_range_", "_ewma_", "_lag_", "_trend_"]
        )
    ]
...
class PredictiveMaintenancePipeline:
    ...
    def _create_model(self):
        ...
        if self.task_type == "classification":
            if self.model_name == "logistic":
                return LogisticRegression(C=self.C, class_weight=self.class_weight_mode, random_state=RANDOM_SEED)
            ...
            if self.model_name == "linear":
                ...
                return RandomForestRegressor(
                    n_estimators=self.n_estimators, max_depth=self.max_depth, random_state=RANDOM_SEED
                )
            elif self.model_name == "xgboost" and HAS_XGB:
    ...
    def fit(self, X: pd.DataFrame, y: np.ndarray) -> "PredictiveMaintenancePipeline":
        ...
        if (
            ...
            and self.class_weight_mode == "balanced"
            and HAS_XGB
        ):
    ...
    def evaluate(self, X: pd.DataFrame, y: np.ndarray) -> Dict[str, float]:
        ...
        if self.task_type == "classification":
            ...
            costs = np.where(
                errors < 0, np.abs(errors) * underestimate_penalty, np.abs(errors)  # Underestimate
            )  # Overestimate
...
def build_parser():
    parser = argparse.ArgumentParser(description="Module 5.2 Predictive Maintenance Production Pipeline CLI")
    subparsers = parser.add_subparsers(dest="command", required=True)

    # Train subcommand
    train_parser = subparsers.add_parser("train", help="Train a predictive maintenance model")
    train_parser.add_argument("--dataset", default="synthetic_maintenance", help="Dataset name")
    ...
    train_parser.add_argument(
        "--task", default="classification", choices=["classification", "regression"], help="Task type"
    )
    train_parser.add_argument("--target", default="event_in_24h", help="Target column name")
    train_parser.add_argument("--horizon", type=int, default=24, help="Prediction horizon in hours")
    train_parser.add_argument("--use-smote", action="store_true", help="Apply SMOTE oversampling (classification only)")
    train_parser.add_argument("--smote-k-neighbors", type=int, default=5, help="SMOTE k-neighbors parameter")
    train_parser.add_argument("--no-class-weight", action="store_true", help="Disable class weight balancing")
    train_parser.add_argument(
        "--class-weight", default="balanced", choices=["balanced", "balanced_subsample"], help="Class weight strategy"
    )
    train_parser.add_argument(
        "--threshold-method", default="youden", choices=["youden", "cost_based"], help="Threshold optimization method"
    )
    train_parser.add_argument("--cost-fp", type=float, default=1.0, help="Cost of false positive")
    train_parser.add_argument("--cost-fn", type=float, default=10.0, help="Cost of false negative")
    train_parser.add_argument("--n-estimators", type=int, default=100, help="Number of estimators for ensemble methods")
    train_parser.add_argument("--max-depth", type=int, default=6, help="Maximum depth for tree-based models")
    train_parser.add_argument("--learning-rate", type=float, default=0.1, help="Learning rate for boosting models")
    train_parser.add_argument("--C", type=float, default=1.0, help="Regularization parameter for linear models")
    ...
    eval_parser.add_argument("--dataset", default="synthetic_maintenance", help="Dataset name")
    ...
    pred_parser.add_argument("--input-json", help="JSON string with input features")
    pred_parser.add_argument("--input-file", help="File containing JSON input features")
...
def main(argv: Optional[List[str]] = None):
    ...
    try:
        ...
        error_result = {"status": "error", "error": str(e), "command": args.command}
