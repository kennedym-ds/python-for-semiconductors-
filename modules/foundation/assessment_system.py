...
class ModuleAssessment:
    ...
    @classmethod
    def load_from_json(cls, filepath: Union[str, Path]) -> "ModuleAssessment":
        """
        Load assessment directly from a JSON question bank file.

        Args:
            filepath: Path to the JSON question bank file

        Returns:
            ModuleAssessment instance with questions loaded
        """
        ...
        assessment.questions = loaded_questions
        return assessment
    ...
    def _load_questions(self) -> None:
        """Load questions for the module from JSON files."""
        # Try to load from assessments directory
        assessments_dir = Path(__file__).parent.parent.parent / "assessments"
        module_dir = assessments_dir / self.module_id

        if not module_dir.exists():
            # Fall back to sample questions for demo
            self.questions = self._get_sample_questions()
            return

        # Load all JSON files in the module directory
        question_files = list(module_dir.glob("*.json"))

        if not question_files:
            # No question files found, use sample questions
            self.questions = self._get_sample_questions()
            return
        ...
        for filepath in question_files:
            try:
                ...
                # Validate that this is the correct module
                if data.get("module_id") != self.module_id:
                    continue
                ...
                print(f"Warning: Failed to load questions from {filepath}: {e}")
                continue

        self.questions = loaded_questions if loaded_questions else self._get_sample_questions()

    def _get_sample_questions(self) -> List[Question]:
    ...
    def run_practical_assessment(self, student_id: str = "default_user") -> AssessmentResult:
        ...
        result = AssessmentResult(
            ...
            question_results=[
                {"task": "data_analysis", "score": 90, "feedback": "Excellent data preprocessing and visualization"}
            ],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Assessment System for Python for Semiconductors Learning Series")
    ...
    assess_parser.add_argument(
        "--type", choices=["knowledge", "practical", "both"], default="both", help="Assessment type"
    )
    ...
    progress_parser.add_argument("--format", choices=["json", "summary"], default="summary", help="Output format")
