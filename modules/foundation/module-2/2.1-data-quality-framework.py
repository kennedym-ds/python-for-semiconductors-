...
class DataQualityFramework:
    ...
    def assess_accuracy(self) -> Dict:
        ...
        accuracy['outlier_analysis'] = {
            ...
            'outlier_rate_iqr_pct': (total_outliers_iqr / total_numeric_values) * 100 if total_numeric_values > 0 else 0,
            'outlier_rate_zscore_pct': (total_outliers_zscore / total_numeric_values) * 100 if total_numeric_values > 0 else 0,
        ...
        if len(numeric_cols) > 0:
            ...
            if ranges:
                ...
                consistency['scale_analysis'] = {
                    ...
                    'range_ratio': float(max(range_values) / min(range_values)) if min(range_values) > 0 else float('inf'),
                    'min_scale': float(min(scale_values)),
                    'max_scale': float(max(scale_values)),
                    'scale_ratio': float(max(scale_values) / min(scale_values)) if min(scale_values) > 0 else float('inf'),
                    'columns_needing_scaling': [col for col, range_val in ranges.items() 
    ...
    def assess_validity(self) -> Dict:
        ...
        validity['format_validity'] = {
            'finite_values_pct': float((np.isfinite(self.df[numeric_cols]).sum().sum() / self.df[numeric_cols].size) * 100) if len(numeric_cols) > 0 else 100,
            'non_null_pct': float((self.df[numeric_cols].notna().sum().sum() / self.df[numeric_cols].size) * 100) if len(numeric_cols) > 0 else 100,
    ...
    def _check_value_ranges(self) -> Dict:
        ...
        range_check['reasonable_range_pct'] = (range_check['reasonable_ranges'] / len(numeric_cols)) * 100 if len(numeric_cols) > 0 else 100
        return range_check
    ...
    def generate_recommendations(self, report_data: Dict) -> List[Dict]:
        ...
        if 'scale_analysis' in consistency and consistency['scale_analysis']['scale_ratio'] > self.thresholds.scale_ratio_warning:
            recommendations.append({
    ...
    def calculate_overall_score(self, report_data: Dict) -> float:
        ...
        weights = {'completeness': 0.3, 'accuracy': 0.3, 'validity': 0.2, 'uniqueness': 0.2}
...
def main():
    ...
    parser.add_argument('--format', '-f', default='json', choices=['json', 'yaml', 'html'], help='Output format')
