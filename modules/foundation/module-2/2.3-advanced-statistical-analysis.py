...
class ANOVAAnalyzer:
    ...
    def check_assumptions(self, response: str, factors: List[str]) -> Dict:
        ...
        try:
            ...
            groups = [self.data[self.data[factors[0]] == level][response].dropna() 
    ...
    def one_way_anova(self, response: str, factor: str) -> Dict:
        ...
        try:
            # Basic ANOVA
            groups = [self.data[self.data[factor] == level][response].dropna() 
    ...
    def multi_way_anova(self, response: str, factors: List[str], 
    ...
    def factorial_analysis(self, response: str, factors: List[str]) -> Dict:
        ...
        try:
            ...
            sorted_effects = dict(sorted(effect_magnitudes.items(), 
    ...
    def power_analysis(self, effect_size: float, n_groups: int, 
        ...
        try:
            ...
            results = {
                ...
                'power_curve': {'sample_sizes': sample_sizes.tolist(), 'powers': powers}
            }
    ...
    def _post_hoc_analysis(self, response: str, factor: str) -> Dict:
        ...
        try:
            if self.config['post_hoc'] == 'tukey':
                ...
                return {
                    'method': 'bonferroni',
                    'results': str(bonferroni_results[0])
                }
        except Exception as e:
            return {'error': str(e)}
    ...
    def _plot_one_way_anova(self, output_path: Path):
        ...
        axes[1,1].bar(['Between Groups', 'Within Groups'], 
                     [eta_squared, 1-eta_squared])
        ...
        if len(factors) >= 2:
            ...
            if len(factors) == 2:
                ...
                for col in interaction_data.columns:
                    axes[0].plot(interaction_data.index, interaction_data[col], 
                               marker='o', label=f'{factors[1]} = {col}')
    ...
    def _plot_factorial_analysis(self, output_path: Path):
        ...
        if sorted_effects:
            ...
            colors = ['red' if effects_detail[name]['significant'] else 'blue' 
            ...
            for analysis_type, results in self.results.items():
                if 'error' not in results:
                    ...
                    if analysis_type == 'one_way_anova':
                        ...
                        f.write(f"Effect size: {results['eta_squared']:.4f} ({results['effect_size_interpretation']})\n")
...
def main():
    ...
    parser.add_argument('--output', '-o', type=str, default='anova_report.html',
                       help='Output report file')
    parser.add_argument('--config', '-c', type=str,
                       help='Configuration file (YAML)')
    
    # Analysis arguments
    parser.add_argument('--response', '-r', type=str, required=True,
    ...
    parser.add_argument('--analysis-type', '-t', type=str, 
    ...
    parser.add_argument('--groups', type=int,
                       help='Number of groups for power analysis')
