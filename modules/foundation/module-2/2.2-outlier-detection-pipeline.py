...
class LOFDetector(OutlierDetector):
    ...
    def __init__(self, config: OutlierDetectionConfig, 
                 n_neighbors: int = None, contamination: float = None):
...
class OutlierDetectionPipeline:
    ...
    def _calculate_consensus(self):
        ...
        logger.info(f"Consensus: {consensus_outliers.sum()} outliers "
...
class OutlierAlertSystem:
    ...
    def process_outlier_detection(self, outlier_results: Dict, 
                                timestamp: datetime, context: Dict = None) -> Optional[Dict]:
        ...
        if alert_level:
            alert = {
                ...
                'outlier_methods': {k: v for k, v in outlier_results.items() 
...
def create_synthetic_secom_data(n_samples: int = 1567, n_features: int = 590, 
                               outlier_rate: float = 0.05) -> pd.DataFrame:
...
def main():
    ...
    parser.add_argument('--recipe-column', type=str, 
                       help='Column name for recipe information')
    ...
    try:
        ...
        if args.realtime:
            ...
            for i in range(len(data)):
                ...
                if streaming_detector.baseline_stats:  # Only after baseline is established
                    ...
                    if result['alert']:
                        print(f"🚨 ALERT at index {i}: {result['alert']['level']} "
        ...
        feature_columns = [col for col in data.columns 
                          if col not in ['recipe', 'timestamp', 'target']]
