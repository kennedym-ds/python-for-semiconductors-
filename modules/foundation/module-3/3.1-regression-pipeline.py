...
def generate_yield_process(n=800, seed: int = RANDOM_SEED) -> pd.DataFrame:
    ...
    yield_pct = (70 + 0.05*(temp-450) - 1.5*(pressure-2.5)**2 + 0.04*flow + 0.2*time +
                 0.0005*(temp-450)*(flow-120) + noise)
...
class RegressionPipeline:
    ...
    def _build_model(self):
        ...
        if self.model_name == 'elasticnet':
            return ElasticNet(alpha=self.alpha, l1_ratio=self.l1_ratio, random_state=RANDOM_SEED, max_iter=10000)
    ...
    def build(self, n_features: int):
        ...
        if self.use_feature_selection:
            steps.append(('select', SelectKBest(score_func=f_regression, k=min(self.k_best, n_features))))
    ...
    def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray,
                        tolerance: float = 2.0, spec_low: float = 60, spec_high: float = 100,
                        cost_per_unit: float = 1.0) -> Dict[str, float]:
...
def action_train(args):
    ...
    pipeline = RegressionPipeline(model=args.model, alpha=args.alpha, l1_ratio=args.l1_ratio,
                                  k_best=args.k_best, pca_components=args.pca_components,
...
def action_predict(args):
    ...
    meta_dict = asdict(pipeline.metadata) if pipeline.metadata else None
    print(json.dumps({'prediction': float(pred), 'input': record, 'model_meta': meta_dict}, indent=2))
...
def build_parser():
    ...
    p_train.add_argument('--model', default='ridge', choices=['ridge','lasso','elasticnet','linear','rf'])
    ...
    p_train.add_argument('--pca-components', type=float, default=0.95, help='PCA components (float for variance or int)')
    p_train.add_argument('--no-feature-selection', action='store_true', help='Disable SelectKBest feature selection')
