...
class ClassificationPipeline:
    ...
    def _build_estimator(self):
        if self.model_name == 'logistic':
            return LogisticRegression(max_iter=1000, class_weight=self.class_weight_mode, C=self.C, random_state=RANDOM_SEED)
        ...
        if self.model_name == 'tree':
            return DecisionTreeClassifier(max_depth=self.max_depth, class_weight=self.class_weight_mode, random_state=RANDOM_SEED)
...
def action_train(args):
    ...
    pipe = ClassificationPipeline(model=args.model,
...
def action_predict(args):
    ...
    print(json.dumps({'prediction': pred, 'probability': float(probs), 'threshold': pipe.fitted_threshold, 'input': record}, indent=2))
...
def build_parser():
    ...
    p_train.add_argument('--model', default='logistic', choices=['logistic','linear_svm','tree','rf','gb'])
    p_train.add_argument('--use-smote', action='store_true', help='Apply SMOTE oversampling (requires imbalanced-learn)')
    ...
    p_train.add_argument('--min-precision', type=float, help='Minimum precision constraint for threshold selection')
    p_train.add_argument('--min-recall', type=float, help='Minimum recall constraint for threshold selection')
    p_train.add_argument('--C', type=float, default=1.0, help='Inverse regularization strength for logistic/linear SVM')
