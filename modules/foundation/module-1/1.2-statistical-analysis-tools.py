...
class HypothesisTestAnalyzer(StatisticalAnalyzer):
    ...
    def one_sample_ttest(
        self, data: np.ndarray, null_value: float, alternative: str = "two-sided"
    ) -> StatisticalResult:
        """
        ...
        *groups : variable number of array-like
        ...
        return StatisticalResult(
            ...
            raw_data={
                ...
                "degrees_freedom": (
                    len(validated_groups) - 1,
                    sum(group_sizes) - len(validated_groups),
                ),
            },
        )
...
class DistributionAnalyzer(StatisticalAnalyzer):
    ...
    def fit_distribution(
        ...
        if distribution_name not in self.available_distributions:
            raise ValueError(
                f"Distribution '{distribution_name}' not available. "
...
class YieldAnalyzer:
    ...
    def monte_carlo_yield_simulation(
        self,
        mean: float,
        std_dev: float,
        specs: SpecificationLimits,
    ...
    def yield_sensitivity_analysis(
        self,
        base_mean: float,
        base_std: float,
        specs: SpecificationLimits,
...
def generate_sample_data(
    n_samples: int = 1000,
    parameter_type: str = "threshold_voltage",
    mean: float = 650,
    std_dev: float = 20,
