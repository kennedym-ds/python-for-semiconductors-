...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('wafer_analysis.log'),
...
class WaferAnalyzer:
    ...
    def load_wafer_data(self, file_path: Union[str, Path], 
    ...
    def calculate_basic_metrics(self, wafer_data: np.ndarray, 
                               wafer_id: str = "Unknown") -> WaferMetrics:
        """
    ...
    def _detect_ring_pattern(self, wafer_data: np.ndarray) -> bool:
        ...
        for radius in range(5, max_radius, 5):
            mask = self._create_ring_mask(wafer_data.shape, center_h, center_w, 
                                        radius-2, radius+2)
            if np.any(mask):
        ...
        for i in range(1, len(radial_yields)-1):
            if (radial_yields[i-1] - radial_yields[i] > 0.2 and 
                radial_yields[i+1] - radial_yields[i] > 0.2):
                return True
        
        return False
    
    def _create_ring_mask(self, shape: Tuple[int, int], center_h: int, 
                         center_w: int, inner_radius: int, outer_radius: int) -> np.ndarray:
    ...
    def generate_visualization(self, wafer_data: np.ndarray, metrics: WaferMetrics,
        ...
        fig.suptitle(f'Wafer Analysis: {metrics.wafer_id} (Yield: {metrics.yield_percentage:.2f}%)', 
    ...
    def _plot_failure_patterns(self, patterns: List[str], ax) -> None:
        ...
        if display_patterns:
            bars = ax.barh(y_pos, [1] * len(display_patterns), 
                          color=['red' if 'loss' in p.lower() or 'low' in p.lower() 
                                else 'green' if 'excellent' in p.lower() or 'normal' in p.lower()
                                else 'orange' for p in display_patterns])
...
Analysis Time: {metrics.timestamp.split('T')[1][:8]}
    ...
    def export_results(self, metrics: WaferMetrics, output_path: str, 
...
def main():
    ...
    parser.add_argument('--wafer-id', '-w', type=str, default='Unknown', 
                       help='Wafer identifier')
    ...
    parser.add_argument('--edge-width', type=int, default=5,
                       help='Edge region width for analysis')
