...
from .schemas import PredictionRequest, PredictionResponse, HealthResponse, ModelInfoResponse, ErrorResponse
...
MODEL_PATH = os.getenv(
    "MODEL_PATH", "/home/runner/work/python-for-semiconductors-/python-for-semiconductors-/temp_models/model.joblib"
)
METADATA_PATH = os.getenv(
    "METADATA_PATH", "/home/runner/work/python-for-semiconductors-/python-for-semiconductors-/temp_models/metadata.json"
)
    ...
    try:
        ...
        if model_path.exists():
            ...
            temp_models_dir = Path(
                "/home/runner/work/python-for-semiconductors-/python-for-semiconductors-/temp_models"
            )
...
def make_prediction(input_data: Dict[str, float]) -> Dict[str, Any]:
    ...
    if model is None:
        ...
        return {"prediction": round(mock_yield, 2), "confidence": 0.85, "model_used": "mock_model"}
...
async def root():
    ...
    return {"service": SERVICE_NAME, "version": API_VERSION, "status": "running", "docs": "/docs"}
...
async def health_check():
    ...
    return HealthResponse(
        status=status, uptime_seconds=round(uptime, 2), model_loaded=model is not None, version=API_VERSION
    )
...
async def predict(request: PredictionRequest):
    ...
    try:
        ...
        return PredictionResponse(
            ...
            metadata={"model_used": result.get("model_used", "unknown"), "timestamp": time.time()},
        )
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    return JSONResponse(
        ...
        content=ErrorResponse(error=exc.detail, status_code=exc.status_code, path=str(request.url.path)).dict(),
    )
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=500,
        content=ErrorResponse(error="Internal server error", status_code=500, path=str(request.url.path)).dict(),
    )
