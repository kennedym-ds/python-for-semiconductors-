"""
Validation script for Module 4.3 - Multilabel Classification assessment.
"""
...
def main():
    """Validate Module 4.3 assessment file."""
    print("=" * 80)
    print("MODULE 4.3 ASSESSMENT VALIDATION - MULTILABEL CLASSIFICATION")
    print("=" * 80)

    file_path = "assessments/module-4/4.3-questions.json"
    path = Path(file_path)
    ...
    if valid:
        ...
        if (
            ...
            and result["multiple_choice"] == expected_mc
            and result["coding_exercise"] == expected_coding
            and result["conceptual"] == expected_conceptual
        ):
            print("\n" + "=" * 80)
            print("✅ ALL VALIDATIONS PASSED!")
            print(f"\n   Module 4.3 (Multilabel Classification) is COMPLETE!")
            ...
            print(
                f"   Found: {result['total']} total ({result['multiple_choice']} MC, {result['coding_exercise']} coding, {result['conceptual']} conceptual)"
            )
            print("=" * 80)
            sys.exit(1)
    else:
