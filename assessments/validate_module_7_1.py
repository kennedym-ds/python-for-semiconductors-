#!/usr/bin/env python3
"""Validate Module 7.1 assessment structure."""
...
def validate_module_7_1():
    """Validate Module 7.1 question bank."""

    file_path = Path("assessments/module-7/7.1-questions.json")
    ...
    print(f"\n📝 Question Types:")
    print(f"   Multiple Choice: {types.get('multiple_choice', 0)}")
    print(f"   Coding Exercises: {types.get('coding_exercise', 0)}")
    print(f"   Conceptual: {types.get('conceptual', 0)}")

    # Calculate distribution
    mc_pct = (types.get("multiple_choice", 0) / total) * 100
    coding_pct = (types.get("coding_exercise", 0) / total) * 100
    conceptual_pct = (types.get("conceptual", 0) / total) * 100
    ...
    if total == 25:
        print(f"   ✅ Question count: {total} (Target: 25)")
        checks_passed += 1
    else:
        print(f"   ⚠️  Question count: {total} (Target: 25)")
